...
then
	...
	db_exist=`mysql --user=$MYSQL_USER  --password=$MYSQL_PASSWORD  --skip-column-names -e "$mysql_cmd"`
	if [ "$?" != "0" ]
	then
		...
		if [ "" == "$db_exist" ]
		then
			printf "mysql --user=$MYSQL_USER  --password=$MYSQL_PASSWORD $pre_dot < $pre_dot.sql\n"
			mysql --user=$MYSQL_USER  --password=$MYSQL_PASSWORD $pre_dot < $pre_dot.sql
